...
| **Base de conhecimento de design + front-end + imagens** | `docs/DESIGN-KB.md` (conceitos) · `docs/DESIGN-RESEARCH.md` (fatos verificados na web: CWV, formatos, números) |
...
- **`public/visual-inspector.html`** — inspeciona qualquer SVG: overlays de grid/colunas/terços, viewports (+ OG 1200), extração de paleta, matriz de contraste, ferramenta de cor, harmonias, checklist. Importa o `design-kit.js`. Ferramentas extras: **📚 Docs** (renderiza DESIGN-KB/DESIGN-RESEARCH inline), **🎨 Tokens** (lê os tokens reais do app em `colors.css` e mostra por **modo** — padrão / alto contraste / fonte grande — com contraste-chave ink×paper e export CSS; é o "Dev Mode" dos nossos tokens), **simulador de daltonismo** (protan/deuter/tritan/cinza via feColorMatrix), **escala tipográfica + espaçamento** (base/razão), **export de paleta** (CSS vars / JSON). **Web embutida:** pesquisa (DuckDuckGo) e lê/extrai paleta de páginas reais via proxies keyless (r.jina.ai + allorigins) — útil quando WebSearch/WebFetch da IA estiverem indisponíveis. Detalhes na seção 16 do `DESIGN-KB.md`.
- **Figma (espelho dos tokens):** o sistema vive em arquivo Figma como **variáveis** (primitivo + alias semântico) + componente em auto layout — `https://www.figma.com/design/CgPtWozHjShqmezG7r3SzD`. Útil para inspecionar/comparar; a fonte de verdade no app continua sendo `colors.css` + `design-kit.js`.
...
## Estado atual
60 cenas interativas + 225 imagens de missão com o design final (gradientes/sombra/sheen), inspecionadas. Gerador refinado e integrado ao `design-kit.js` (acessibilidade automática). Base de design (`DESIGN-KB.md` + `DESIGN-RESEARCH.md` + `design-kit.js` + `visual-inspector.html`) disponível. Tokens do app espelhados na Figma (variáveis + alias + auto layout) e lidos no inspector por modo (padrão/alto contraste/fonte grande). A11y do app: kill-switch de motion completo + alto contraste automático via `prefers-contrast: more`.
